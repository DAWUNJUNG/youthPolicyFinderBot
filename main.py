...
def chatbotProxy(kakaoUid, request, botQueue, controlInfo):
    ...
    if "시작하기" in userMessage:
        ...
        if userMessage == '정부 중앙부처 정책':
            returnData = step1CenterGoverment(kakaoUid)
        elif userMessage == '지자체 정책':
            returnData = step1City(kakaoUid)
        else:
            returnData = commandErrorMessage()
    elif controlInfo['step'] == 1:
        if controlInfo['searchType'] == 'centerGovernment':
            if userMessage == '전체 검색':
                returnData = step2CenterGovermentAll(kakaoUid)
            elif userMessage == '선택 검색':
                returnData = step2CenterGovermentPart(kakaoUid)
            else:
                returnData = step1ErrorMessage()
        else:
            if userMessage == '도시 지정하기':
                returnData = step2City(kakaoUid)
            else:
                returnData = step1ErrorMessage()
            ...
            "outputs": [
                {
                    "textCard": {
                        ...
                        "description": "전체 중앙부처를 기준으로 검색하시려면 전체 검색을 클릭해주세요!\n\n특정 중앙부처를 검색하시려면 아래 중앙부터 목록 중 하나의 부처를 검색해주세요!",
                        "buttons": [
                            {
                                "action": "message",
                                "label": "전체 검색",
...
def step2CenterGovermentAll(kakaoUid):
    setSearchStep(kakaoUid, 2, 'centerGovernment')
...
def step2CenterGovermentPart(kakaoUid):
    setSearchStep(kakaoUid, 2, 'centerGovernment')
...
def step2City(kakaoUid):
    setSearchStep(kakaoUid, 2, 'city')
